...
initSim('D:/PAMStudio_dev/git/BdD_Sorghum_10geno.accdb', 'BAMA2014S1621B')

#detailled load
#initSimDetails('D:/PAMStudio_dev/git/BdD_Sorghum_10geno.accdb', 'Sotuba-2011-D1', '06SB15_12fev13', 'SotubaPCDA', '270130', '2011/05/01', '2011/12/31')
...
robsRed = rsamara::rcpp_reduceVobs(vObs, resulttmp)
resRed = rsamara::rcpp_reduceResults(resulttmp, vObs)

#Parameters to estimate
ParamOfInterest <- c("leaflengthmax", "phyllo")
...
Optim_Samara_funct <- function(p){
  ...
  diff = ((robsRed - res)/robsRed)^2
  return(sum(sqrt((colSums(diff, na.rm=T))/(colSums(!is.na(diff)))),na.rm=T))
}

##DE
resOptimDE <- DEoptim(Optim_Samara_funct, lower=Bounds[,1], upper=Bounds[,2], DEoptim.control(VTR=0,itermax=30, strategy=2))

##plots results
varlist = c("totbiom", "leafdm","panicledm", "lai")
...
# for (v in varlist) {
#   yLimMax = max(res[[v]],robsRed[[v]])
#   yLimMin = min(res[[v]],robsRed[[v]])
#   plot(res$dap, res[[v]], type="l", ylab=v, ylim=c(yLimMin,yLimMax))
#   points(robsRed$dap, robsRed[[v]], type="p", col="red")
# }

for (v in varlist) {
  plot(res$dap, res[[v]], type="l", ylab=v)
  points(robsRed$dap, robsRed[[v]], type="p", col="red")
}
